...
struct workQueue{
  private:
    ...
    std::mutex m;
    std::condition_variable apt;
    bool finish = false;

  public:
    std::experimental::optional<std::string> remove(){
        std::unique_lock<std::mutex> lock(m);
        apt.wait(lock, [this]{return !workQ.empty() || finish;});
        auto filename = workQ.front();
        workQ.pop_front();
        return filename;
    }
...
struct hashTable{
  private:
    std::unordered_map<std::string, std::list<std::string>> uHashTable;
    std::mutex m;
    std::condition_variable apt;
  ...
  public:
    auto find(std::string filename){
      {
        std::unique_lock<std::mutex> lock(m);
        return uHashTable.find(filename);
      }
    }

    std::list<std::string> *vals(std::string key){
      {
        std::unique_lock<std::mutex> lock(m);
        return &uHashTable[key];
      }
    }

    void insert(std::string filename, std::list<std::string> l){
      std::unique_lock<std::mutex> lock(m);
      uHashTable.emplace(std::make_pair(filename, l));
    }

    auto getEnd(){
      std::unique_lock<std::mutex> lock(m);
      return uHashTable.end();
    }
...
static void process(const char *file, std::list<std::string> *ll, hashTable * theTable, workQueue * workQ) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (theTable->find(name) != theTable->getEnd()) { continue; }
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = theTable.vals(name);
...
int main(int argc, char *argv[]) {
  ...
  if(crawler_threads != nullptr){
    num_threads = std::stoi(crawler_threads);
  }
  ...
  for (auto i = 0; i < num_threads; i++){
    threads.emplace_back([](){
      while ( workQ.size() > 0 ) {
        std::experimental::optional<std::string> optional;
        optional = workQ.remove();
        if(!optional){
          exit(-1);
        }
        auto filename = optional.value();

        if (theTable.find(filename) == theTable.getEnd()) {
          fprintf(stderr, "Mismatch between table and workQ\n");
          return -1;
        }

        // 4a&b. lookup dependencies and invoke 'process'
        process(filename.c_str(), theTable.vals(filename),&theTable,&workQ);
      }
      return 0;
    });
  }
